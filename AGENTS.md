...
AIでアイキャッチを生成する場合は、[アイキャッチ画像生成ワークフロー](_docs/eyecatch-image-generation-workflow.md)に従う。
...
- 1枚に載せる系列は3つまで、目盛りは5本まで、1行のラベルは日本語12文字・英数字28文字まで。入らない場合はパネルを分けるか本文へ逃がす
- 棒グラフの軸は必ず0から始める。途中で切ると差が誇張される
- 縦軸を2本持つ図は作らない。単位が違う指標はパネルを分ける
- 数値の出典と年月を図中の脚注に入れる。`_data/stale_watch.yml`の対象記事は、本文を更新するときに図も再生成する
- 生成後に`python3 _scripts/charts/check.py`を通し、そのうえで実機幅で目視確認する

```markdown
![altテキスト](/assets/images/posts/YYYY-MM-DD-slug/figure.svg){: .chart}
```
...
- 内部ドキュメントを `_` で始まらないディレクトリへ置かない。`docs/` のような名前だと
  Jekyll がページとして公開し、サイトマップにも載って検索対象になる。置き場所は `_docs/`
