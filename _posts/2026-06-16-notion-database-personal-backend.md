...
![データベースの中にデータソースがあり、その下にページとプロパティが並ぶNotionの構造](/assets/images/posts/2026-06-16-notion-database-personal-backend/data-model.svg){: .chart}
...
![自作アプリとNotionの画面が同じデータソースを読み書きする経路](/assets/images/posts/2026-06-16-notion-database-personal-backend/access-paths.svg){: .chart}
...
無料プランのバージョン履歴については、2025 年以降は実質無制限になっている（以前は 7 日分の制限があった）。大きなファイルは外部ストレージに置き、Notion には URL や参照だけを持たせる形になる。
