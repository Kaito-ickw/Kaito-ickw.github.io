...
開発はRustで、AGPL-3.0-or-laterのオープンソース版と、組織向けの商用ライセンスを併用するデュアルライセンス方式を取っている。2026年7月11日時点でGitHubスター数は15.3k、最新リリースはv0.7.5（2026年7月21日）。2026年8月末時点での最新リリースはv0.8.2。インストールは`curl -fsSL https://herdr.dev/install.sh | sh`で行う（Windows版はベータ扱い）。
