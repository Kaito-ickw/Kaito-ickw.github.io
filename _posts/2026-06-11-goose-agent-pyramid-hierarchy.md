...
Apache 2.0ライセンスで公開されており、Block社内の開発者が実際に使うために作られたツールが外部に出てきた形だ。その後プロジェクトはLinux Foundation傘下のAgentic AI Foundation（AAIF）に移管され、コミュニティ主導で開発が継続されている。
...
```bash
# Gooseのインストール
curl -fsSL https://github.com/aaif-goose/goose/releases/latest/download/install.sh | sh
...
Gooseが関係するのは、その「誰でも使えるOSSの入口」という性格だ。特定のIDEやサブスクリプションに縛られず、自前のLLMキーで動かせる。Blockの開発者が実際に使うために作られた出自が、「実用に耐えるかどうか」の一定の証左になっている。
...
調達方針で選ぶなら、Anthropic契約があればClaude Codeが体験として安定している。自社でLLMを管理したい、複数プロバイダを使い分けたい場合にGooseかopencodeが候補に上がる。コーディング専用に割り切るならopencode、コーディング以外の自動化タスクも含めたいならGooseの選択肢が広がる。opencodeのモード設計とモデル選択については[LLM を選べる OSS コーディングエージェント opencode]({% post_url 2026-06-08-opencode-guide %})で扱っている。
...
階層を薄くした先で誰が顧客に向き合うのかという問いは、[Forward Deployed Engineer のロールとビジネスモデル]({% post_url 2026-06-11-fde-role-business-model %})で別に整理している。
...
- [aaif-goose/goose](https://github.com/aaif-goose/goose) ── Gooseの公式リポジトリ（Linux Foundation AAIF）
- [anomalyco/opencode](https://github.com/anomalyco/opencode) ── opencodeの公式リポジトリ（Anomaly）
