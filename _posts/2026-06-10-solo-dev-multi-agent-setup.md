...
複数のエージェントを並行して動かすようになると、worktreeとタスクの対応をターミナル側で持たせたくなる。この前提で作られたターミナルについては[AIエージェント前提のターミナル Herdr と cmux]({% post_url 2026-07-11-ai-agent-terminal-herdr-cmux %})で見ている。
