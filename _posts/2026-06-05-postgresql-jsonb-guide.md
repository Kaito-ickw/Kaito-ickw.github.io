...
実用上、**書き込みより読み取りが多く、検索を行うなら JSONB 一択**。`json` 型を選ぶのはキーの順序や重複を完全に保持しなければならない特殊なケースに限られる。MySQL の JSON 型との違いは[MySQL の JSON と PostgreSQL の JSONB の違いと使い分け]({% post_url 2026-08-28-mysql-json-vs-postgresql-jsonb %})で比較している。
...
JSONB は「何でも入る箱」として使いすぎると後からクエリの最適化が困難になる。**固定部分はリレーショナル、可変部分は JSONB** という役割分担を意識することが、長期的に保守しやすい設計につながる。リレーショナル側に昇格したカラムのスキーマ変更をコードで管理する方法は、[Alembic 入門: SQLAlchemy と使う DB マイグレーション管理]({% post_url 2026-06-07-alembic-database-migration-guide %})で整理している。
