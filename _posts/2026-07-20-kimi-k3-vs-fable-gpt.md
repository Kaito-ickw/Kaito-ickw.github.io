...
7月時点から状況が動いている。Artificial Analysisは指数を改定し（v4.1.1、8月20日）、K3のスコアは60になった。GLM-5.3と並び、オープンウェイトでは引き続き首位である。同じ時期に発表されたAlibabaのモデルについては[Alibaba Qwen3.8-Max Preview 速報]({% post_url 2026-07-20-qwen3-8-max-preview %})に書いた。またClaude Opus 5などさらに新しいフロンティアモデルも登場しているため、本文の表は現在のリーダーボードではなく7月のフロンティアのスナップショットとして読んでほしい。K3のAPI価格は$3 / $15のまま変わっていない。

## 追記（2026年9月1日）

GPT-5.6 Solの価格が8月21日に改定された。本文の表では入力$5・出力$30と記載しているが、現在のプロモーション価格は入力**$4**・出力**$20**（2026年11月21日まで）になっている。K3（$3/$15）との価格差は引き続き存在するが、差は縮まっている。Claude Fable 5（$10/$50）の価格に変更はない。
